...
eval_interval=400
seed=1234
output_dir='warmup_policy'


python3 -m torch.distributed.launch --nproc_per_node=2 policy_warmup_main.py --epoch $epoch \
 ...
 --lr $lr --warmup_ratio $warmup_ratio --grad_acc $grad_acc --model_to_use $model_to_use --beta1 $beta1 --beta2 $beta2 \
  --log_interval $log_interval --eval_interval $eval_interval --seed $seed  --num_steps $num_steps --output_dir $output_dir --task $task
