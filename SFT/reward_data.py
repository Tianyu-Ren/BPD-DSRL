...
def load_data(tokenizer, split='train', max_length=384, task='squad1'):
    ...
    if task == 'newsqa':
        dataset = dataset.select_columns(['context_truncation',
                                          'question', 'answer']).rename_column('context_truncation',
                                                                               'context')

    def tokenize(x):
        context, question, answer = x['context'], x['question'], x['answer']
        output_ids = tokenizer(question + '<hl>' + answer).input_ids
